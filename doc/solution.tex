...
A \emph{page} is long-term JSON-encoded textual database used for archival of records. Each page contains a link to a previous page, forming an append-only public ledger known as a page-chain. In section \ref{sec:Algorithms} I describe how an individual machine can maintain its own page-chain and in section \ref{sec:TorApplication} I detail how nodes in the Tor network can maintain a common page-chain. In any fully-connected network where all participants know everyone's public keys, the page-chain becomes publicly confirmable and can be used for distributed DNS. 
...
\section{Application Within Tor}
