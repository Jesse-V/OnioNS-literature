...
\begin{figure}[htbp]
	...
	\caption{A Tor client, Alice, and a hidden service, Bob, communicate by mating two Tor circuits at a \emph{rendezvous point} (RP). Bob maintains circuits (red) to his \emph{introduction points} (IPs) and advertises IPs and $ \mathrm{PK}_{B} $ to the Tor network. When Alice obtains Bob's address through a backchannel, she selects and builds a circuit (blue) to \emph{RP} and other circuit (purple) to one of Bob's IPs. She then tells $ \mathit{IP}_{B} \mathcal{E}_{\mathrm{PK}_{B}}(\mathit{RP}) $. Then Bob construct a circuit (green) to \emph{RP} and Alice and Bob can communicate with bi-directional anonymity.\cite{overlier2006locating}}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{../images/Tor/onion_2014-10_2015-04.pdf}
	\caption{The number of unique .onion addresses seen in Tor's distributed hashtable between January through April 2015.\cite{kadianakis2015extrapolating}\cite{TorMetrics}}
...
\begin{center}
  \begin{tabular}{ | l | l | } \hline
    $ L_{Q} $ & the size of the Quorum \\
    $ L_{T} $ & the number of routers in the Tor network \\
