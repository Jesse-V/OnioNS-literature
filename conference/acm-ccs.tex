...
\category{E.2}{Data Structures}{Data storage representations}[Linked representations]
...
This paper is divided into four main sections. First, we define our design objectives, the challenges that we seek to overcome, and explain why existing works do not meet our goals. We also define our threat model, which closely matches Tor's model with one additional assumption. Second, we describe the system overview and define several key protocols. Last, we evaluate our implementation prototype and show that it can access a hidden service under a meaningful domain name.

\section{Problem Statement}

We design OnioNS as a plugin to Tor and integrate it into Tor's network. However, Tor's privacy-enhanced environment introduces a distinct set of security requirements that must be met by any additional infrastructure. We must preserve user privacy and avoid compromising other areas of the Tor network. Additionally, we seek to achieve all three properties of Zooko's Triangle (section \ref{sec:ZookosTriangle}) and providing a mechanism for authenticated denial-of-existence (section \ref{sec:authDenialIntro}).
...
% todo: 512 isn't in legend, make font bigger
\begin{figure*}
	\centering
