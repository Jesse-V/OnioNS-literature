...
The Quorum must regenerate $ T $ every $ \Delta T $ hours to include new Records. Then Alice needs only pre-fetch $ T $ and its $ \mathit{sig}_{T} $s at least every $ \Delta T $ hours to ensure that she can authenticate new Records during the Domain Query. Thus $ \Delta T $ is the primary factor in the speed of Record propagation: Alice cannot authenticate or verify denial-of-existence claims on Records newer than $ \Delta T $. However, the networking cost is $ \mathcal{O}(n \Delta T) $. Alice must also fetch the $ L_{Q} \mathit{sig}_{T} $s from all Quorum nodes and assert that $ T $ is signed by the largest set of nodes maintaining the same Page, in correspondence with our last security assumption.
...
However, Merkle trees do not support updates without complete rebuilding. Future work may explore more efficient data structures such as the hash tables proposed by Papamanthou et el.\cite{papamanthou2008authenticated}, which is noteworthy for achieving constant query cost and sublinear update cost.
