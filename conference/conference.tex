...
%\subsection{Overview}
%
%We propose the Onion Name System (OnioNS) as an abstraction layer to onion service addresses and introduce ``.tor'' as a new pseudo-TLD for this purpose.
%
...
%	\item Introduces a barrier-of-entry that encourages the utilization of domain names and the availability of the underlying onion services.
...
% If the lottery required a constant amount of computational work for each lottery ticket, then a computationally-powerful adversary, Craig, could easily generate many tickets and quickly place the statistics in his favor. We counter this potential for abuse 
...
% todo: add a note that fragmented adversaries also allows innocent users to win names
...
% \textbf{I don't think that the above upper bound is correct. Equation 2 is correct for the lower-bound. How do I proceed and demonstrate the significance of $ \nWinners $? I should be able to show that large $ \nWinners $ provides a defense against mass-generation of tickets.}
...
We tested the performance of our OnioNS-HS and OnioNS-client software on two machines, $ M_{a} $ and $ M_{b} $. $ M_{a} $ is powered by an Intel Core2 Quad Q9000 2.00 GHz Penryn CPU from late 2008 and $ M_{b} $ contains an Intel i7-6700K 4.0 GHz Skylake CPU from 2015, representing low-end and high-end consumer-grade computers, respectively. Both machines were hosted on a 16 Mbits connection, representing typical residential speed. We measured the performance of OnioNS-client. We averaged 200 samples of the CPU wall-time required for both machines to validate a record.
