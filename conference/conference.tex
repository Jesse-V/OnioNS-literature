...
\begin{enumerate}
	...
	\item Alice uses $ \mathcal{R}(\mathcal{G}(i)) $ to select $ \min(\mathrm{size}(\mathit{qc}), L_{Q}) $ Quorum nodes from $ \mathit{qc} $ with the probability of selecting router $ x $ determined by Equation \ref{eq:qSelection}.
\end{enumerate}
...
% $ d_{b} / \ceil{\frac{j}{\left\vert{W_{i}}\right\vert}} $, where $ d_{b} $ is the base difficulty, $ \left\vert{W_{i}}\right\vert $ is the cardinality of $ W_{i} $, and $ j $ is the ticket's index, $ 1 \leq i \leq \left\vert{W_{i}}\right\vert $.
